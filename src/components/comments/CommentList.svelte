<script lang="ts">
  ...
  {:else}
    <div class="border-t border-zinc-100 dark:border-base-800 pt-2">
        ...
        <div class="w-full text-left mt-6">
          <div class="flex items-start gap-3">
            <div class="flex-shrink-0">
              ...
              {:else if comment.user_id}
                <button
                  ...
                  class="w-10 h-10 rounded-xl flex items-center justify-center bg-zinc-100 dark:bg-base-800 text-zinc-700 dark:text-base-300 font-nouvelr-semibold cursor-pointer hover:opacity-80 transition-opacity"
                >
                  {(comment.user_fullname || comment.user_name).charAt(0).toUpperCase()}
                </button>
              {:else}
                <div class="w-10 h-10 rounded-xl flex items-center justify-center bg-zinc-100 dark:bg-base-800 text-zinc-700 dark:text-base-300 font-nouvelr-semibold">
            ...
            <div class="flex-grow">
              <div class="flex justify-between items-start">
                <div>
                  {#if comment.user_id}
                    ...
                    >
                      <h4 class="font-nouvelr-semibold text-base-800 dark:text-base-50">{comment.user_fullname}</h4>
                    </button>
                    <button
                      type="button"
                      onclick={(e) => {
                ...
                <span class="text-xs text-zinc-400 dark:text-zinc-500 font-nouvelr">{formatSimpleDate(comment.created_at)}</span>
              </div>
              <button
                type="button"
              ...
              >
                <p class="mt-2 text-base-700 dark:text-base-300 font-display text-[14px] line-clamp-3">{comment.content}</p>
              </button>
               ...
               <div class="flex items-center gap-5">
                ...
                <button
                  type="button"
                  onclick={() => navigateToComment(comment.id)}
                  class="mt-3 flex items-center gap-1 text-xs text-blue-600 dark:text-blue-400 font-display hover:text-blue-300 cursor-pointer"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
