...
export default function EditorActionButtons({
  ...
  const handleCancel = async () => {
    if (onCancel) {
      ...
      const ok = await confirmDialog({
        title: "Çıxış",
        message:
          "Dəyişikliklər yadda saxlanılmayacaq. Çıxmaq istədiyinizə əminsiniz?",
        variant: "neutral",
  ...
  const handleSaveDraft = () => {
    if (saveBtnState.handleSaveDraft) {
      saveBtnState.handleSaveDraft();
    }
  };
  ...
  const draftDisabled =
    saveBtnState.isSaving ||
    (!saveBtnState.title && !saveBtnState.editorContent);
