  ...
  <ul class="stream-video-card-grid grid list-none grid-cols-1 gap-6 p-0 md:grid-cols-2 lg:grid-cols-4">
    {#each videos as video (video.guid)}
      <li id={anchorScroll ? video.guid : undefined} class={anchorScroll ? "scroll-mt-24" : ""}>
        <button
          type="button"
          class="group relative block w-full cursor-pointer overflow-hidden rounded-lg text-left outline-none ring-offset-2 transition-shadow focus-visible:ring-2 focus-visible:ring-slate-900 dark:focus-visible:ring-base-200 dark:ring-offset-base-950"
...
{#if modalOpen && selected}
  ...
  <div
    class="fixed inset-0 z-[100] flex items-stretch justify-center bg-black/85 backdrop-blur-sm sm:items-center sm:p-4"
    onclick={backdropClick}
    role="presentation"
  >
    <div
      class="relative flex h-[100dvh] max-h-[100dvh] w-full max-w-6xl flex-col overflow-hidden bg-white shadow-2xl outline-none dark:bg-base-900 lg:h-[min(92dvh,880px)] lg:max-h-[min(92dvh,880px)] lg:flex-row lg:items-stretch sm:rounded-xl"
      role="dialog"
      aria-modal="true"
    ...
    >
      <button
        type="button"
        class="absolute right-3 top-3 z-[110] flex size-9 cursor-pointer items-center justify-center rounded-xl border border-base-200/90 bg-white/95 text-base-600 shadow-sm backdrop-blur-md transition-all duration-200 hover:border-rose-200 hover:bg-base-50 hover:text-rose-600 active:scale-[0.97] dark:border-base-600 dark:bg-base-800/95 dark:text-base-300 dark:shadow-black/20 dark:hover:border-rose-500/40 dark:hover:bg-base-700 dark:hover:text-rose-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-500/45 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-offset-base-900 sm:right-4 sm:top-4 sm:size-10"
        onclick={closeModal}
        aria-label="Bağla"
      >
      ...
      >
        ...
        >
          ...
          <p class="mt-1 text-sm text-base-500 dark:text-base-400">
            ...
            {#if selected.category}
              <span class="text-base-400"> · {selected.category}</span>
            {/if}
          </p>
          {#if selected.streamVideoId}
            <p class="mt-2 flex items-center gap-1.5 text-xs text-base-500 dark:text-base-400 font-nouvelr">
              <svg class="size-3.5 shrink-0 opacity-80" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
              </svg>
              <span>
                <span class="font-nouvelr-semibold text-base-700 dark:text-base-300">{formatViewCount(siteViewCountFor(selected))}</span>
              </span>
            </p>
          {/if}
          {#if selected.description?.trim()}
            <div
              class="mt-4 rounded-xl border border-base-200/80 bg-base-50/90 px-3.5 py-3 dark:border-base-700/80 dark:bg-base-800/50 sm:px-4 sm:py-3.5"
            >
              <p
                class="mb-2 text-[11px] font-nouvelr-semibold uppercase tracking-wider text-base-400 dark:text-base-500"
              >
                Təsvir
              </p>
              <p
                class="m-0 text-sm leading-[1.65] text-base-700 dark:text-base-200 font-nouvelr whitespace-pre-wrap [overflow-wrap:anywhere]"
              >
        ...
        <div class="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-3 sm:px-5">
          {#if !selected.streamVideoId}
            <p class="text-sm leading-relaxed text-base-500 dark:text-base-400 font-nouvelr">
              Şərhlər yalnız saytda qeydiyyata alınmış videolar üçündür. Bu klip birbaşa CDN siyahısındandırsa,
              şərh əlavə etmək mümkün deyil.
            </p>
            ...
            <ul class="space-y-4">
              {#each comments as c (c.id)}
                <li class="flex gap-3">
                  ...
                  {:else}
                    <div
                      class="flex size-9 shrink-0 items-center justify-center rounded-xl bg-base-200 text-sm font-nouvelr-semibold text-base-600 dark:bg-base-800 dark:text-base-300"
                    >
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-baseline gap-x-2 gap-y-0">
                      <span class="text-sm font-nouvelr-semibold text-base-900 dark:text-base-50">
                        {c.users?.fullname || c.users?.username || "İstifadəçi"}
                      </span>
                      {#if !c.user_id}
                        <span class="text-xs text-base-400 font-nouvelr">Qonaq</span>
                      {/if}
                      <span class="text-xs text-base-400">{formatSimpleDate(c.created_at)}</span>
                    </div>
                    <p class="mt-1 whitespace-pre-wrap text-sm leading-relaxed text-base-700 dark:text-base-300 font-nouvelr">
                      {c.content}
                    </p>
                  </div>
                </li>
